...
def delete_standard_clause(clause_id):
    ...
    # ✅ FIXED: return is now correctly inside this function
...
def email_report(contract_id):
    ...
    try:
        ...
        current_app.logger.info(f"📧 MAIL_USERNAME set: {bool(current_app.config.get('MAIL_USERNAME'))}")
        current_app.logger.info(f"📧 MAIL_PASSWORD set: {bool(current_app.config.get('MAIL_PASSWORD'))}")

        clauses = Clause.query.filter_by(
            contract_id=contract.id
        ).order_by(Clause.segment_index).all()

        current_app.logger.info(f"📧 Clauses found: {len(clauses)}")
...
def _send_analysis_email(to_email, filename, clauses):
    # ✅ FIXED: using current_app.config instead of os.environ
...
Log in to view the full interactive analysis.
    ...
    # ✅ Using port 465 with SMTP_SSL (most reliable for Gmail)
